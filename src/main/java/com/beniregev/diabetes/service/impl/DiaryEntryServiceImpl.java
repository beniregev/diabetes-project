...
public class DiaryEntryServiceImpl implements DiaryEntryService {
    ...
    @Override
    public List<DiaryEntryDto> create(List<DiaryEntryPostRequest> listDiaryEntriesPostRequest) {
        List<DiaryEntryDto> listDiaryEntryDto = DiaryEntryMapper.INSTANCE.listDiaryEntryPostRequestToListDiaryEntryDto(listDiaryEntriesPostRequest);
        List<DiaryEntry> listDiaryEntry = DiaryEntryMapper.INSTANCE.listDiaryEntryDtoToListDiaryEntry(listDiaryEntryDto);
        List<DiaryEntry> listCreated = diaryEntryRepository.saveAll(listDiaryEntry);
        return DiaryEntryMapper.INSTANCE.listDiaryEntryToListDiaryEntryDto(listCreated);
    }

    @Override
    public List<DiaryEntryDto> create(LocalDate defaultDate, List<DiaryEntryPostRequest> listDiaryEntriesPostRequest) {
        List<DiaryEntry> listDiaryEntry = listDiaryEntriesPostRequest.stream()
                .map(e -> new DiaryEntry(
                        LocalDateTime.of(defaultDate, e.getMeasurement().getDefaultTime()),
                        e.getMeasurement(),
                        e.getResult()))
                .toList();
        List<DiaryEntry> listCreated = diaryEntryRepository.saveAll(listDiaryEntry);
